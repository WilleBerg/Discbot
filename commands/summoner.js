...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option => option.setName('name').setDescription('Search')),
	async execute(interaction) {
		...
		const { id, puuid, name, summonerLevel, status} = await fetch(`https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${term}?api_key=${riotApiKey}`)
			.then(response => response.json());
        
        console.log(status);
        if(typeof status !== 'undefined'){
            console.log("status is defined");
            await interaction.editReply({ content: `There is no user with the name "${term}"!`, ephemeral: true});
            ...
            if(list.length !== 0){
                ...
                const testEmbed = new MessageEmbed()
                    ...
                    .addFields(
                        ...
                        {name : `Gametype:`, value: `${info.gameType}`, inline: true},
                        {name : `Result:`, value : (`${win}`), inline : true},
                    )
                    .addFields(
                        {name : 'Champion:', value : champ, inline : true},
                        {name : 'KDA:' , value : (deaths < 10 ? `${kills}/${deaths}/${assists}` : `${kills}/${deaths}/${assists} <:pepeLaugh2:852905715676872765>`), inline : true}
                    )
                interaction.editReply({embeds: [testEmbed]});
            } else {
                await interaction.editReply({ content: `The user "${term}" has no latest game!`, ephemeral: true});
            }
        }
	},
};
