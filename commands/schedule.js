...
export const schedule = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName("type")
                .setDescription(
                    "Search for specific type of lesson. Lecture is default if type left blank"
                )
        ),
    ...
    async execute(interaction) {
        ...
        console.log(
            "Found date closest to todays date at index " + firstIndex + "!\n"
        );
        ...
        if (nmbr >= schedule.length) {
            if (type != null) {
                await interaction.editReply({
                    content: `Found no event in schedule with type ${type}`,
                    ephemeral: true,
                });
            } else {
                await interaction.editReply({
                    content: `Found no event! (This is a bug)`,
                    ephemeral: true,
                });
            }
            ...
            //bad solution!! find how to end with reply
            ...
            const testEmbed = new MessageEmbed()
                ...
                .setFooter(
                    type != "next"
                        ? `I assumed you meant ${assumed.nameTranslated}`
                        : ""
                );
...
function findByType(hours, minutes, todays, object, firstIndex) {
    ...
    for (let i = firstIndex; i < schedule.length; i++) {
        ...
        if (
            nameTranslated.toLowerCase() === Typ.toLowerCase() ||
            nameTranslated.toLowerCase() == Kurs__1.toLowerCase()
        ) {
